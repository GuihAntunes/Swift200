...
class ContaGotasViewController: UIViewController {
    ...
    class func apresentaCom( _ quemChamou:AquelesQueMudamDeCor,
                             corInicial:UIColor = UIColor.blue){
        ...
        UIApplication.shared.keyWindow?.rootViewController?.present(contaGotas, animated: true, completion: nil)
    ...
    @IBAction func valorMudou(_ alvo:UISlider){
    ...
    @IBAction func aplicar(_ sender: AnyObject) {
        quemMeChamou?.recebeNovaCor(amostra.backgroundColor!)
        dismiss(animated: true, completion: nil)
    }
    
    @IBAction func cancela(_ sender: AnyObject) {
        dismiss(animated: true, completion: nil)
    }
    
    func definirCorInicial(_ cor:UIColor){
